...
def informacje_o_eksperymencie():
    run = True
    while run:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                menu = pygame.draw.rect(main_screen, dark_green, (540, 280, 100, 60))
                mouse = pygame.mouse.get_pos()
                click = pygame.mouse.get_pressed()

                if 540 + 100 > mouse[0] > 540 and 280 + 60 > mouse[1] > 280:
                    pygame.draw.rect(main_screen, green, (540, 280, 100, 60))
                    if click[0] == 1:
                        intro_screen()
                else:
                    pygame.draw.rect(main_screen, dark_green, (540, 280, 100, 60))

                button_description = pygame.font.Font("AutourOne-Regular.otf", 20)
                textSurf, textRect = text_objects(("Menu"), button_description)
                textRect.center = ((540 + (100 / 2)), (280 + (60 / 2)))
                main_screen.blit(textSurf, textRect)
